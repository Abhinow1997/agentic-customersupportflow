...
export async function GET({ url }) {
  try {
    ...
    // ITEM table was re-seeded with real Walmart product data (SKs start at 813)
    // but STORE_RETURNS still holds legacy SR_ITEM_SK values (2-50).
    // Fix: rank ITEM rows by I_ITEM_SK and join on rank = SR_ITEM_SK so every
    // ticket gets a real, deterministic product without touching the source data.
    ...
    if (rows.length > 0) {
      ...
      console.log('[tickets] sample product:', rows[0].I_PRODUCT_NAME, '|', rows[0].I_CATEGORY);
    } else {
...
function mapRowToTicket(row, idx) {
  ...
  const email      = row.C_EMAIL_ADDRESS     ?? `customer-${ticketNum}@unknown.com`;
  const preferred  = row.C_PREFERRED_CUST_FLAG === 'Y';

  // REASON dimension
  const reasonDesc = row.R_REASON_DESC       ?? `Reason #${row.SR_REASON_SK ?? 'unknown'}`;

  // DATE_DIM
  const returnDate = row.D_DATE              ?? null;

  // ITEM dimension -- enriched with real Walmart product data
  const productName   = row.I_PRODUCT_NAME   ?? `Item #${row.SR_ITEM_SK ?? 'unknown'}`;
  const category      = row.I_CATEGORY       ?? 'General';
  const categoryFull  = row.I_CATEGORY_FULL  ?? category;
  const itemClass     = row.I_CLASS          ?? '';
  const itemBrand     = row.I_BRAND          ?? '';
  const itemPrice     = parseFloat(row.I_CURRENT_PRICE ?? 0);
  const itemListPrice = parseFloat(row.I_LIST_PRICE    ?? itemPrice);
  const itemDesc      = (row.I_ITEM_DESC     ?? '').slice(0, 300);
  const itemUrl       = row.I_PRODUCT_URL    ?? '';
  ...
  return {
    ...
    item: {
      ...
      listPrice:    itemListPrice.toFixed(2),
      desc:         itemDesc,
      url:          itemUrl,
      returnQty,
    },
...
function buildTriage(reasonDesc, category, itemPrice, returnAmt, returnQty, netLoss, preferred) {
  ...
  if (isDamaged && isElectronics) {
    ...
    if (isHighValue && preferred) {
      ...
      action = 'refund'; actionLabel = 'Process Refund';
      actionRationale = 'Customer no longer needs the item -- standard refund.';
    }
  } else if (isLateSlow) {
    action = 'refund_or_reship'; actionLabel = 'Refund or Reship';
  ...
  return { action, actionLabel, actionRationale, refundSignal, policyRef, flags, priorityOverride };
}
